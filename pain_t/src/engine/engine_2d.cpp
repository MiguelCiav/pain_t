...
engine_2d::engine_2d(int width, int height, const std::string &title)
...
void engine_2d::init() {
  ...
  colors[ImGuiCol_CheckMark] = ImVec4(0.28f, 0.45f, 0.80f, 1.00f);
  colors[ImGuiCol_SliderGrab] = ImVec4(0.28f, 0.45f, 0.80f, 1.00f);
  colors[ImGuiCol_SliderGrabActive] = ImVec4(0.18f, 0.35f, 0.70f, 1.00f);
...
void engine_2d::setup_canvas() {
  float vertex[] = {1.0f,  1.0f,  1.0f, 1.0f, 1.0f,  -1.0f, 1.0f, 0.0f,
...
void engine_2d::key_callback(GLFWwindow *window, int key, int scancode,
                             int action, int mods) {
  if (key < 0 || key >= GLFW_KEY_LAST)
    return;
  engine_2d *engine =
      static_cast<engine_2d *>(glfwGetWindowUserPointer(window));
...
void engine_2d::mouse_button_callback(GLFWwindow *window, int button,
                                      int action, int mods) {
...
void engine_2d::cursor_pos_callback(GLFWwindow *window, double xpos,
                                    double ypos) {
  engine_2d *engine =
      static_cast<engine_2d *>(glfwGetWindowUserPointer(window));
  if (engine) {
