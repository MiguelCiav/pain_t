...
bool line::can_fill() const {
    return false;
}

void line::draw_fill() {
    throw std::logic_error("Cannot fill a line");
}
...
bool line::contains_point(double x, double y) const {
    return false;
}

bounding_box line::get_bounding_box() const {
    if (control_points.size() != 2) {
...
std::string line::get_type_tag() const {
    return "line";
}
